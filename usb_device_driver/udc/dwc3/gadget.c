...
static int dwc3_alloc_trb_pool(struct dwc3_ep *dep)
{
	...
	dep->trb_pool = usb_dma_malloc(
			sizeof(struct dwc3_trb) * DWC3_TRB_NUM, /*cyl???*/
			&dep->trb_pool_dma);
	if (!dep->trb_pool) {
		dev_err("failed to allocate trb pool for %s\n",
				dep->name);
		return -ENOMEM;
	}
...
static void dwc3_free_trb_pool(struct dwc3_ep *dep)
{
	...
	usb_dma_mem_free(dep->trb_pool, dep->trb_pool_dma);
		...
		bool ignore, bool restore)
{
	...
	if (usb_endpoint_xfer_control(desc) || usb_endpoint_xfer_int(desc))
		...
		const struct usb_endpoint_descriptor *desc)
{
	...
	if (!ep || !desc || desc->bDescriptorType != USB_DT_ENDPOINT) {
		dev_err("dwc3: invalid parameters\n");
		return -EINVAL;
	}

	if (!desc->wMaxPacketSize) {
		dev_dbg("dwc3: missing wMaxPacketSize\n");
		return -EINVAL;
	}
...
static int dwc3_gadget_ep_disable(struct usb_ep *ep)
{
	...
	if (!(dep->flags & DWC3_EP_ENABLED)) {
		dev_warn("%s is already disabled\n",
				dep->name);
		return 0;
	}
		...
		unsigned length, unsigned last, unsigned chain, unsigned node)
{
	...
	if (!req->trb) {
		...
		req->start_slot = dep->trb_enqueue;
	}
		...
		int start_new)
{
	...
	if (ret < 0) {
		dev_dbg("failed to send STARTTRANSFER command\n");
...
static int dwc3_gadget_ep_queue(struct usb_ep *ep, struct usb_request *request)
{
	...
	if (!dep->endpoint.desc) {
		dev_dbg("trying to queue request %p to disabled %s\n",
				request, ep->name);
		ret = -ESHUTDOWN;
		goto out;
	}
	...
	if (req->dep != dep) {
		 dev_warn("request %p belongs to '%s'\n",
	...
	if (dwc->pullups_connected == false) {
		dev_warn(
			 "DWC gadget is disconnected and ignore queuing.\n");
		ret = -ESHUTDOWN;
		goto out;
	}
...
//	trace_dwc3_ep_dequeue(req);
	...
	if (r != req) {
		...
		dev_err("request %p was not queued to %s\n",
				request, ep->name);
		ret = -EINVAL;
		goto out0;
	}
...
int __dwc3_gadget_ep_set_halt(struct dwc3_ep *dep, int value, int protocol)
{
	...
	if (usb_endpoint_xfer_isoc(dep->endpoint.desc)) {
		dev_err("%s is of Isochronous type\n", dep->name);
		return -EINVAL;
	}
	...
	if (value) {
		if (!protocol && ((dep->direction && dep->flags & DWC3_EP_BUSY) ||
			...
			dev_dbg("%s: pending request, cannot halt\n",
					dep->name);
			return -EAGAIN;
		}
		...
		if (ret)
			dev_err("failed to set STALL on %s\n",
					dep->name);
		else
		...
		if (ret)
			dev_err("failed to clear STALL on %s\n",
					dep->name);
		else
...
static int dwc3_gadget_wakeup(struct usb_gadget *g)
{
	...
	unsigned long		timeout, current_tick;
	unsigned long		flags;
	...
	if (speed == DWC3_DSTS_SUPERSPEED) {
		dev_dbg("no wakeup on SuperSpeed\n");
		ret = -EINVAL;
		goto out;
	}
	...
	case DWC3_LINK_STATE_U3:	/* in HS, means SUSPEND */
		break;
	default:
		dev_dbg("can't wakeup from link state %d\n",
				link_state);
		ret = -EINVAL;
		goto out;
	}
	...
	if (ret < 0) {
		dev_err("failed to put link in Recovery\n");
		goto out;
	}
	...
	if (DWC3_DSTS_USBLNKST(reg) != DWC3_LINK_STATE_U0) {
		dev_err("failed to send remote wakeup\n");
		ret = -EINVAL;
	}
...
static int dwc3_gadget_run_stop(struct dwc3 *dwc, int is_on, int suspend)
{
	...
	/*
	 * The dwc3 device maybe enter suspend mode before function driver start
	 * to run the gadet. So if the dwc3 device has been in suspend state,
	 * just return and leave the running action start from dwc3 resume
	 * action.
	 */
	#ifdef SUSPEND
...
#define USB_INT_NUM 29
	...
	if (dwc->gadget_driver) {
		dev_err("%s is already bound to driver\n",
				dwc->gadget.name);
		ret = -EBUSY;
		goto err1;
	}
...
static int dwc3_gadget_stop(struct usb_gadget *g)
{
	 ...
	 * free the gadget irq, it will not shutdown this gadget irq. Then the
	 * gadget driver can not handle the end transfer command complete event
	 * after free the gadget irq, which will hang the system to crash.
	 *
		...
		const struct dwc3_event_depevt *event, int status)
{
	...
	if ((trb->ctrl & DWC3_TRB_CTRL_HWO) && status != -ESHUTDOWN)
		 ...
		 */
		dev_err("%s's TRB (%p) still owned by HW\n",
				dep->name, trb);
	...
	if (dep->direction) {
		if (count) {
			...
			if (trb_status == DWC3_TRBSTS_MISSED_ISOC) {
				dev_dbg("incomplete IN transfer %s\n",
						dep->name);
				/*
				...
				dev_err("incomplete IN transfer %s\n",
						dep->name);
				status = -ECONNRESET;
			}
		} else {
		...
		const struct dwc3_event_depevt *event)
{
	...
	case DWC3_DEPEVT_STREAMEVT:
		if (!usb_endpoint_xfer_bulk(dep->endpoint.desc)) {
			dev_err("Stream event for non-Bulk %s\n",
					dep->name);
			return;
		}
...
err4:
	...
	usb_dma_mem_free(dwc->ep0_bounce, dwc->ep0_bounce_addr);

err3:
	usb_mem_free(dwc->setup_buf);

err2:
	usb_dma_mem_free(dwc->ep0_trb, dwc->ep0_trb_addr);

err1:
	usb_dma_mem_free(dwc->ctrl_req, dwc->ctrl_req_addr);
